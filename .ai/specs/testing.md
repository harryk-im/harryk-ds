...
우리 프로젝트에는 이미 **Storybook + Chromatic**(시각 회귀) 레이어가 있어요.
그래서 새로 추가하는 **Vitest 레이어는 "외형"이 아니라 "행동과 계약"만 검증**해요.
두 레이어가 겹치면 유지보수가 어려워지니, 아래 경계를 반드시 지켜주세요.

| 레이어 | 도구 | 책임 | 여기서 하지 말 것 |
|------|------|------|------------------|
| **시각 회귀** | Chromatic (기존) | 픽셀, 색상, 레이아웃, variant 외형 | 로직·상호작용 검증 |
| **행동/계약** | Vitest + RTL + vitest-axe (신규) | 렌더 시맨틱, props→속성/aria 변화, 상호작용, 접근성 | 픽셀·색상·해시 클래스명 검증 |

> **핵심 원칙**: "사용자가 보고 만지는 계약"을 검증해요. "어떻게 구현했는지"는 검증하지 않아요.
...
- ⚠️ **금지**: variant가 특정 vanilla-extract 클래스명을 갖는지 검증하지 마세요. 해시가 바뀌면 깨져요. 외형은 Chromatic의 몫이에요.
...
- **픽셀/색상 검증**: Chromatic의 책임이에요. Vitest에서 하지 마세요.
